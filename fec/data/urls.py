...
#below line is for testing -mremove before any commit
...
urlpatterns = [
    ...
    url(r'^data/elections/(?P<office>\w+)/(?P<state>\w+)/(?P<district>\w+)/(?P<cycle>[0-9]+)/$', views.elections, name='elections-house'),
    url(r'^data/elections/(?P<office>\w+)/(?P<state>\w+)/(?P<cycle>[0-9]+)/$', views.elections, name='elections-senate'),
