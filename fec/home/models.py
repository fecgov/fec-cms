...
def get_content_section(page):
    ...
    content_sections = [
        slugs.get(ancestor.slug)
        for ancestor in ancestors
        if slugs.get(ancestor.slug) != None  # noqa
    ]
...
def log_user_save(sender, **kwargs):
    ...
    if kwargs.get('update_fields'):
        ...
        logger.info(
            "User change: username {0} by instance {1}".format(
                kwargs.get('instance').get_username(), kwargs.get('instance')
            )
        )
...
def log_revisions(sender, **kwargs):
    ...
    logger.info(
        'Page modified: {0} by user {1}'.format(kwargs.get('instance'), username)
    )
...
def user_groups_changed(sender, **kwargs):
    ...
    if kwargs.get('action').split('_')[0] == 'post':
        for each in kwargs.get('pk_set'):
            action = 'to' if kwargs.get('action').split('_')[1] == 'add' else 'from'
            logger.info(
                "User change: User {0} was {1} {2} group {3}".format(
                    kwargs.get('instance').get_username(),
...
class Author(models.Model):
    ...
    author_group = models.CharField(
        ...
        blank=True,
        help_text="Not required: Only choose this if you \
...
class PageAuthors(models.Model):
    ...
    author = models.ForeignKey(
        Author, related_name='+', null=True, on_delete=models.SET_NULL
    )
    role = models.CharField(
        max_length=255, choices=constants.author_roles.items(), default='author'
    )
...
class RecordPageTag(TaggedItemBase):
    content_object = ParentalKey('home.RecordPage', related_name='tagged_items')
...
class RecordPage(ContentPage):
    ...
    related_section_url = models.CharField(max_length=255, blank=True, default='/data/')
    monthly_issue = models.CharField(max_length=255, blank=True, default='')
    monthly_issue_url = models.CharField(max_length=255, blank=True, default='')
...
class DigestPage(ContentPage):
    ...
    search_fields = ContentPage.search_fields + [index.FilterField('date')]
...
class HomePageBannerAnnouncement(Page):
    ...
    date_active = models.DateTimeField(
        blank=False,
        help_text='Set active date-active here and \
        leave Go-Live and Expiry fields blank in the Settings-Tab above.',
    )
    date_inactive = models.DateTimeField(
        null=True,
        blank=False,
        help_text='Set date-inactive here and \
        leave Go-Live and Expiry fields blank in the Settings-Tab above.',
    )
...
class AlertForEmergencyUseOnly(Page):
    ...
    alert_date_inactive = models.DateTimeField(
        null=True,
        blank=False,
        help_text='Set date-inactive here and \
        leave Go-Live and Expiry fields blank in the Settings-Tab above.',
...
class CustomPage(Page):
    ...
    citations = StreamField(
        [('citations', blocks.ListBlock(CitationsBlock()))], null=True, blank=True
    )
    ...
    show_contact_link = models.BooleanField(
        ...
        null=False,
        blank=False,
        choices=[(True, 'Show contact link'), (False, 'Do not show contact link')],
    )
...
class PressLandingPage(Page):
    ...
    option_blocks = StreamField([('option_blocks', OptionBlock())])
...
class DocumentPage(ContentPage):
    ...
    category = models.CharField(
        max_length=255, choices=constants.report_child_categories.items(), null=True
    )
    content_panels = Page.content_panels + [
        FieldPanel('date'),
...
class DocumentFeedPage(ContentPage):
    ...
    category = models.CharField(
        max_length=255, choices=constants.report_parent_categories.items(), null=True
    )
    content_panels = Page.content_panels + [
...
class ReportsLandingPage(ContentPage, UniqueModel):
    ...
    document_feeds = StreamField(
        [('document_feed_blurb', DocumentFeedBlurb())], null=True, blank=True
    )
...
class AboutLandingPage(Page):
    hero = stream_factory(null=True, blank=True)
    sections = StreamField([('sections', OptionBlock())], null=True)
...
class CommissionerPage(Page):
    ...
    party_affiliation = models.CharField(
        max_length=2,
        choices=(('D', 'Democrat'), ('R', 'Republican'), ('I', 'Independent'),),
    )
...
class CollectionPage(Page):
    ...
    show_contact_card = models.BooleanField(
        ...
        null=False,
        blank=False,
        choices=[(True, 'Show contact card'), (False, 'Do not show contact card')],
    )
    content_panels = Page.content_panels + [
        StreamFieldPanel('body'),
...
class ResourcePage(Page):
    ...
    date = models.DateField(default=datetime.date.today)
    intro = StreamField([('paragraph', blocks.RichTextBlock())], null=True, blank=True)
    sidebar_title = models.CharField(max_length=255, null=True, blank=True)
    related_pages = StreamField(
        [('related_pages', blocks.ListBlock(blocks.PageChooserBlock()))],
        null=True,
    ...
    sections = StreamField([('sections', ResourceBlock())], null=True, blank=True)
    citations = StreamField(
        [('citations', blocks.ListBlock(CitationsBlock()))], null=True, blank=True
    )
    related_topics = StreamField(
    ...
    promote_panels = Page.promote_panels + [FieldPanel('category'), FieldPanel('date')]
...
class MeetingPage(Page):
    ...
    meeting_type = models.CharField(
        max_length=2, choices=MEETING_TYPE_CHOICES, default=OPEN
    )
    ...
    imported_html = StreamField(
        [('html_block', blocks.RawHTMLBlock())], null=True, blank=True
    )
        ...
        [
            (
                ...
                blocks.ListBlock(
                    ThumbnailBlock(),
                    icon='doc-empty',
                    template='blocks/related-media.html',
                ),
            ),
        ],
        null=True,
...
class ContactPage(Page):
    contact_items = StreamField([('contact_items', ContactInfoBlock())])
    services_title = models.TextField()
    services = StreamField([('services', blocks.RichTextBlock())])
...
class FullWidthPage(ContentPage):
    ...
    content_panels = ContentPage.content_panels + [StreamFieldPanel('citations')]
