...
def current_commissioners(grid):
    current_commissioners = CommissionerPage.objects.filter(
        term_expiration__isnull=True
    )
    chair_commissioner = (
        current_commissioners.filter(commissioner_title__startswith='Chair')
        .exclude(commissioner_title__contains='Vice')
        .first()
    )
    vice_commissioner = current_commissioners.filter(
        commissioner_title__startswith='Vice'
    ).first()
    other_commissioners = current_commissioners.exclude(
        Q(commissioner_title__startswith='Chair')
