...
def home_page_banner_announcement():
    ...
    banners = (
        HomePageBannerAnnouncement.objects.live()
        .filter(
            active=True, date_active__lte=datetime_now, date_inactive__gt=datetime_now
        )
        .order_by('-date_active')[:2]
    )
    alert_banners = (
        AlertForEmergencyUseOnly.objects.live()
        .filter(
            alert_active=True,
            alert_date_active__lte=datetime_now,
            alert_date_inactive__gt=datetime_now,
        )
        .order_by('-alert_date_active')[:2]
    )

    return {'banners': banners, 'alert_banners': alert_banners}
...
def draft_home_page_banner_announcement():
    ...
    draft_alert_banners = AlertForEmergencyUseOnly.objects.all().order_by(
        '-alert_date_active'
    )

    return {'draft_banners': draft_banners, 'draft_alert_banners': draft_alert_banners}
...
def home_page_news():
    ...
    press_releases_pinned = (
        PressReleasePage.objects.live()
        .filter(
            homepage_hide=False, homepage_pin=True, homepage_pin_start__lte=date.today()
        )
        .order_by('-date')
    )
    records_pinned = (
    ...
    updates = islice(
        chain(
            featured_updates, record, tips_for_treasurer, press_release, weekly_digest
        ),
        4,
    )
