...
function DateFilter(elm) {
  ...
  Inputmask({inputFormat: 'mm/dd/yyyy', placeholder: 'mm/dd/yyyy', oncomplete: this.validate.bind(this)}).mask(this.$minDate);
  Inputmask({inputFormat: 'mm/dd/yyyy', placeholder: 'mm/dd/yyyy', oncomplete: this.validate.bind(this)}).mask(this.$maxDate);
  ...
  this.$elm.on(
    'click',
    '.date-range__grid li',
    this.handleGridItemSelect.bind(this)
  );
...
DateFilter.prototype.validate = function() {
  ...
  var minDateYear = this.$minDate.val()
    ? parseInt(this.$minDate.val().split('/')[2])
    : this.minYear;
  var maxDateYear = this.$maxDate.val()
    ? parseInt(this.$maxDate.val().split('/')[2])
    : this.maxYear;
...
DateFilter.prototype.handleGridItemSelect = function(e) {
  ...
  if (!this.$grid.hasClass('is-invalid')) {
    var $nextItem = this.$grid.hasClass('pick-min')
      ? this.$maxDate
      : this.$submit;
