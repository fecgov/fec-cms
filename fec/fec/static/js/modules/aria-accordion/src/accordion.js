import { default as extend } from './util.js';
...
export default function Accordion(elm, selectors, opts) {
  this.elm = elm;
...
Accordion.prototype.findTriggers = function() {
  const self = this;
  const triggers = [].slice.call(this.elm.querySelectorAll(this.selectors.trigger));
...
Accordion.prototype.toggle = function(elm) {
  const f = elm.getAttribute('aria-expanded') === 'true' ? this.collapse : this.expand;
  f.call(this, elm);
};
...
Accordion.prototype.setStyles = function(content) {
  const prop = content.getAttribute('aria-hidden') === 'true' ? 'none' : 'block';
