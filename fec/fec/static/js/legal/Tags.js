...
function Tags(props) {
  ...
  var citationRequireAll =
    props.query.ao_citation_require_all &&
  ...
  function getTag(name, value) {
    ...
    return (
      <div key={name + '-' + tagText} className="tag__item">
        {tagText}
        <button
          className="button tag__remove"
          onClick={removeQuery(name, value)}
        >
    ...
    if (Array.isArray(query[tagName])) {
      ...
      if (query[tagName].min && query[tagName].max) {
        return (
          <li
            key={tagName}
            className="tag__category tag__category__range--date"
          >
            {[
              getTag(query[tagName].minName, query[tagName].min),
              getTag(query[tagName].maxName, query[tagName].max)
            ]}
          </li>
        );
      } else if (query[tagName].min) {
        return (
          <li key={tagName} className="tag__category">
            {getTag(query[tagName].minName, 'Beginning ' + query[tagName].min)}
          </li>
        );
      } else {
        return (
          <li key={tagName} className="tag__category">
            {getTag(query[tagName].maxName, 'Ending ' + query[tagName].max)}
          </li>
        );
      }
    } else {
  ...
  if (getTagCategories()) {
    return (
      <div className="data-container__tags">
        <div className="row">
          <h3 className="tags__title">
            Viewing <span className="tags__count">{props.resultCount}</span>{' '}
            {getTagCategories().length > 0
              ? ' filtered results for:'
              : ' results'}
          </h3>
        </div>
        <ul className="tags">{getTagCategories()}</ul>
      </div>
    );
  } else {
    return;
