...
class CitationFilter extends React.Component {
  ...
  setSelection(citation) {
    ...
    const syntheticEvent = {
      target: { name: this.props.name, value: citationTags }
    };
  ...
  render() {
    return (
      ...
      >
        <label className="label" htmlFor={this.props.name + '-filter'}>
          {this.props.label}
        </label>
        {this.props.value &&
          this.props.value.map(citationText => {
            return (
              <Checkbox
                key={citationText}
                name={citationText}
                label={citationText}
                checked={true}
                handleChange={() => this.removeCitation(citationText)}
              />
            );
          })}
        {this.props.lastFilter === this.props.name && (
          <div className="filter__message filter__message--success">
            <div>Filter {this.state.lastAction} &#x2713;</div>
            <div>{this.getFilterAddedText()}</div>
          </div>
        )}
        <div
          className="combo combo--search--mini"
          ...
          >
            <div className="tt-dataset tt-dataset-candidate">
              ...
              {this.state.citations.map((citation, index) => {
                return (
                  <div
                    key={citation.citation_text}
                    onMouseDown={() =>
                      this.setSelection(citation.citation_text)
                    }
                    onMouseOver={this.handleMouseOver(index)}
                    className="selectCitation"
                  >
                    <span
                      className={
