...
class LegalSearch extends React.Component {
  constructor(props) {
    ...
    if (
      initState.ao_statutory_citation &&
      !Array.isArray(initState.ao_statutory_citation)
    ) {
      initState.ao_statutory_citation = [initState.ao_statutory_citation];
    }
    if (
      initState.ao_regulatory_citation &&
      !Array.isArray(initState.ao_regulatory_citation)
    ) {
      initState.ao_regulatory_citation = [initState.ao_regulatory_citation];
    }

    if (
      !initState.ao_category &&
      !(initState.ao_is_pending && initState.ao_is_pending === 'true')
    ) {
      initState.ao_category = ['F'];
    } else if (!Array.isArray(initState.ao_category)) {
      initState.ao_category = [initState.ao_category];
    }
    initState.from_hit = initState.from_hit
      ? parseInt(initState.from_hit, 10)
      : 0;
  ...
  getResults(e) {
    ...
    $.getJSON(queryPath.toString(), results => {
      this.setState(
        {
          advisory_opinions: results.advisory_opinions,
        ...
        () => {
          ...
          window.history.pushState(
            URI.parseQuery(queryPath.query()),
            null,
            queryPath.search().toString()
          );
        }
      );
    });
  ...
  render() {
    return (
      <section className="main__content--full data-container__wrapper">
        <div id="filters" className="filters is-open">
          <button
            className="filters__header filters__toggle js-filter-toggle"
            type="button"
          >
          ...
          <div className="filters__content">
            <Filters
              ...
              instantQuery={this.instantQuery}
            />
          </div>
        </div>
        <div id="results-aos" className="content__section data-container">
          <div className="data-container__widgets">
            ...
            <Tags
              ...
              handleRemove={this.instantQuery}
            />
            <div className="u-padding--left u-padding--right">
              <div className="message message--info">
                <p>
                  The advisory opinion search feature includes all FEC advisory
                  opinions &#40;AOs&#41; to date. You can search all FEC AOs
                  using keywords, AO numbers, names of requestors and more. For
                  additional search filters, you can still search AOs using our
                  legacy{' '}
                  <a href="https://saos.fec.gov/saos/searchao" title="Search advisory opinions">AO search</a>.
                </p>
              </div>
            </div>
          </div>
          <SearchResults
            advisory_opinions={this.state.advisory_opinions}
            q={this.state.q}
            loading={this.state.loading}
          />
          <Pagination
            from_hit={this.state.from_hit}
            advisory_opinions={this.state.advisory_opinions}
            resultCount={this.state.resultCount}
            handleChange={this.instantQuery}
          />
        </div>
      </section>
    );
