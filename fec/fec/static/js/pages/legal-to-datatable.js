...
// var dtable = require('datatables.net');
...
const covert_for_sort = function(type, value) {
      ...
      var regex = /\d{2}\/\d{2}\/\d{4}/i; //not using this line
...
Array.from(tables).forEach(table => {
  //$('table').each(function(index, value){
   ...
   if (legal_docs_sorting.hasOwnProperty(table_id)) {
      ...
      for(let i=0; i < table.rows.length; i++) {
        ...
        console.log ('legal_docs_sorting[table_id]:'+ legal_docs_sorting[table_id])
            if (legal_docs_sorting[table_id].hasOwnProperty(j)) {
      ...
      let order_col
      let order_dir
      if (legal_docs_sorting[table_id].hasOwnProperty('ordering')) {
 ...
 //2) HIS ONE JUST APPLIES jQuery datatables to the existing table
      // var myTable = $('.block-table table')
